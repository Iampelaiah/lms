...
export function ForumFeed({ posts, savedPostIds, isAdmin, onVote, onComment, onOpenCreateModal, onToggleSave, onDelete }: ForumFeedProps) {
  ...
  return (
    <main className="space-y-4">
      {/* Sort & Filter Bar */}
      <div className="bg-background border border-border rounded-lg p-2 flex items-center justify-between sticky top-4 z-10 shadow-lg">
        ...
        <div className="flex items-center gap-1 text-foreground/">
          <button 
            ...
            className={`p-1.5 rounded ${viewMode === 'card' ? 'bg-background text-foreground/' : 'hover:bg-background'}`}
          >
            <LayoutGrid className="w-4 h-4" />
          </button>
          <button 
            onClick={() => setViewMode('compact')}
            className={`p-1.5 rounded ${viewMode === 'compact' ? 'bg-background text-foreground/' : 'hover:bg-background'}`}
          >
      ...
      {/* Feed Content */}
      <div className={viewMode === 'card' ? "grid grid-cols-1 md:grid-cols-2 gap-4" : "flex flex-col space-y-4"}>
        <AnimatePresence>
          {sortedPosts
