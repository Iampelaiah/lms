...
export default function GradingEditor({ initialContent, activeAnnotationId, onAnnotationClick, onAddAnnotation }: GradingEditorProps) {
  ...
  return (
    <div className="flex-1 flex flex-col bg-obsidian overflow-hidden relative">
      <style>{`
          ...
          {bubbleMode === 'menu' ? (
            ...
            <div className="w-[300px] p-3 flex flex-col gap-2 relative">
                  ...
                  <div className="max-h-[150px] overflow-y-auto">
                    {mockResources.map(res => (
                      <button 
                        ...
                        className="w-full text-left px-3 py-2 hover:bg-white/10 text-sm text-white/90 flex items-center gap-2 border-b border-white/5 last:border-0"
                      >
                        <span className="w-6 h-6 rounded bg-royal/10 text-royal flex items-center justify-center text-[10px] font-bold">{res.type}</span>
              ...
              <div className="flex justify-end mt-1">
                <Button size="sm" onClick={handleAddAnnotationSubmit} disabled={!inputValue.trim()} className="h-8 gap-1 bg-royal hover:bg-royal/90 text-obsidian">
